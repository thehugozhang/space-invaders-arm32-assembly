...
UpdateLasers:
	...
	updateShotLaserBody:
        ...
        // Skip collision check to save time if bullet from ship greater than 84 y pos.
        // Earliest aliens start are centered at 64.
        cmp r5, #84
        bgt skipCollisionCheck

        // Pass in updated coords and check for collision with enemies.
        // r0 returns 1 if collision and should skip. Otherwise, continue.
        mov r0, r8
        mov r1, r5
        bl OnEnemyCollision
        cmp r0, #1
        beq destroyLaserOnCollision
    ...
    destroyLaserOnCollision:
        // Moves laser to end of screen so not drawn again.
        mov r5, #0x14
        strb r5, [r4, #PIXMAP_YPOS]
	skipUpdateLasers:
		add r10, r10, #1
...
OnEnemyCollision:
    ...
    // Reset r4 to 0 again if no collision detected.
    // mov r4, #0
    // ldr r6, =enemy2_controls
    // ldr r7, =enemy3_controls

    // Reached end of all three loops meaning no collision detected for this
    // individual laser with any of the enemy sprites.
    mov r0, #0
    b no_collision_detected

    // Skip to here on collision because laser can only hit on enemy at a time.
